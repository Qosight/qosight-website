...
document.addEventListener('DOMContentLoaded', function () {
  ...
  projectForm.addEventListener('submit', function (e) {
    ...
    formStatus.textContent = 'Submitting your project request...';

    supabaseClient
      .from('project_inquiries')
      .insert([{
        name: document.getElementById('pf-name').value.trim(),
        discord_username: document.getElementById('pf-discord').value.trim(),
        email: document.getElementById('pf-email').value.trim(),
        community_name: document.getElementById('pf-community-name').value.trim(),
        community_type: document.getElementById('pf-community-type').value.trim(),
        project_type: document.getElementById('pf-project-type').value,
        server_link: document.getElementById('pf-server-link').value.trim() || null,
        help_needed: document.getElementById('pf-help').value.trim(),
        project_description: document.getElementById('pf-description').value.trim()
      }])
      .then(function (result) {
        if (result.error) throw result.error;

        formStatus.style.color = 'var(--accent)';
        formStatus.textContent = "Thanks - your project request has been sent. We'll be in touch soon.";
        projectForm.reset();
      })
      .catch(function (error) {
        console.error('Project inquiry submission error:', error);
        formStatus.style.color = '#E08585';
        formStatus.textContent = 'Something went wrong sending your request. Please try again - your entered information has been kept.';
        // Form is intentionally not reset here so the person doesn't lose what they typed.
      })
      .finally(function () {
  ...
  /* ---------- Admin access (footer) ---------- */
  var adminForm = document.getElementById('admin-access-form');
  var adminStatus = document.getElementById('admin-access-status');

  if (adminForm) {
    adminForm.addEventListener('submit', function (e) {
      e.preventDefault();

      var codeInput = document.getElementById('admin-access-code');
      var code = codeInput.value;
      var submitBtn = adminForm.querySelector('button[type="submit"]');
      ...
      // The real security check happens in Supabase (Auth + RLS), not here.
      // This just signs in with the code as the password for a single fixed
      // admin account. If it succeeds, the session is stored by the
      // Supabase client and admin.html reads that same session on load.
      supabaseClient.auth.signInWithPassword({
        email: 'qosight001@gmail.com',
        password: code
      }).then(function (result) {
        if (result.error) throw result.error;
  ...
  /* ---------- Visit logging (for the admin Recent Visits panel) ----------
     Logs an approximate page visit, city/country from IP, not exact GPS,
     the browser never asks the visitor for permission. If the location
     lookup fails for any reason, the visit still gets logged without it.
     This never blocks page rendering and never shows an error to the
     visitor either way. */
  logVisit();
  ...
  function logVisit() {
    ...
    fetch('https://ipapi.co/json/')
      ...
      .then(function (geo) {
        return supabaseClient.from('site_visits').insert([{
          page: basicVisit.page,
          referrer: basicVisit.referrer,
          user_agent: basicVisit.user_agent,
          city: geo.city || null,
      ...
      .catch(function () {
        supabaseClient.from('site_visits').insert([basicVisit]).catch(function () {});
      });
  }

  /* ---------- Our Work (dynamic portfolio, managed from admin.html) ----------
     Categories and their videos live in Supabase now, in the
     portfolio_categories and portfolio_videos tables, instead of being
     hardcoded here. Adding a category or a video in the admin page changes
     what shows up here automatically, no file edits needed. */
  var workGrid = document.getElementById('work-grid');
  var portfolioModal = document.getElementById('portfolio-modal');
  var portfolioTitleEl = document.getElementById('portfolio-modal-title');
  var portfolioDescEl = document.getElementById('portfolio-modal-description');
  var portfolioListEl = document.getElementById('portfolio-modal-list');
  var portfolioPlayer = document.getElementById('portfolio-modal-player');
  var portfolioVideo = document.getElementById('portfolio-modal-video');
  var portfolioCloseBtn = document.getElementById('close-portfolio-modal');
  var lastPortfolioTrigger = null;

  // Categories can have their own uploaded thumbnail (thumbnail_url from
  // Supabase, set from the admin page). The original 3 categories fall
  // back to these existing repo images if no upload has replaced them yet.
  // Anything with neither just shows a plain text placeholder.
  var portfolioThumbByKey = {
    structure: 'assets/images/work/server-structure.png',
    verification: 'assets/images/work/verification-systems.png',
    rules: 'assets/images/work/rules-community-organization.png'
  };
    ...
    supabaseClient
      ...
      .then(function (result) {
        ...
        (result.data || []).forEach(function (category) {
          workGrid.appendChild(buildWorkCard(category));
        });
      })
      .catch(function (error) {
  ...
  function buildWorkCard(category) {
    ...
    imageDiv.setAttribute('aria-label', category.title + ' preview');

    var fallback = document.createElement('span');
    fallback.className = 'work-thumb-fallback';
    fallback.textContent = category.title;

    var thumbSrc = category.thumbnail_url || portfolioThumbByKey[category.key];
    if (thumbSrc) {
    ...
    typeEl.textContent = category.title;

    var h3 = document.createElement('h3');
    h3.textContent = category.title;

    var pEl = document.createElement('p');
    pEl.textContent = category.description || '';

    var viewBtn = document.createElement('button');
    viewBtn.type = 'button';
    viewBtn.className = 'btn btn-secondary work-view-btn';
    viewBtn.textContent = 'View Work';
    viewBtn.addEventListener('click', function () {
      openPortfolio(category, viewBtn);
    });
  ...
  function stopPortfolioVideo() {
    ...
    portfolioVideo.load();
    portfolioPlayer.style.display = 'none';
  }

  function openPortfolio(category, trigger) {
    lastPortfolioTrigger = trigger;
    portfolioTitleEl.textContent = category.title;
    portfolioDescEl.textContent = category.description || '';
    stopPortfolioVideo();
    portfolioListEl.innerHTML = '<p style="color:var(--text-muted);">Loading videos...</p>';
    portfolioModal.hidden = false;
    document.body.style.overflow = 'hidden';
    portfolioCloseBtn.focus();
    ...
    supabaseClient
      ...
      .then(function (result) {
        if (result.error) throw result.error;
        buildPortfolioList(result.data || [], category.title);
      })
      .catch(function (error) {
        console.error('Error loading portfolio videos:', error);
        portfolioListEl.innerHTML = '<p style="color:var(--text-muted);">Could not load videos right now.</p>';
      });
  }

  function buildPortfolioList(videos, categoryTitle) {
    portfolioListEl.innerHTML = '';

    if (!videos.length) {
      portfolioListEl.innerHTML = '<p style="color:var(--text-muted);">No videos added for this category yet.</p>';
      return;
    }

    videos.forEach(function (video, index) {
      var li = document.createElement('li');
      li.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:0.75rem;border-bottom:1px solid var(--border);padding-bottom:0.6rem;';
      ...
      playBtn.setAttribute('aria-label', 'Play example ' + (index + 1) + ' of ' + categoryTitle + ' work');
      playBtn.addEventListener('click', function () {
        portfolioVideo.src = video.video_url;
        portfolioPlayer.style.display = 'block';
        portfolioVideo.load();
        portfolioVideo.play().catch(function () {});
      });
  ...
  function closePortfolio() {
    ...
    document.body.style.overflow = '';
    stopPortfolioVideo();
    if (lastPortfolioTrigger) lastPortfolioTrigger.focus();
  }
  ...
  if (portfolioCloseBtn) {
    ...
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && !portfolioModal.hidden) closePortfolio();
    });
  }
